import { clampStripWidth, ComicProjectSchema, type ComicProjectData } from "./schema";
 ...
 *  - StripWidth > 0(스트립 문서) → [690,1500] 클램프 + 모든 페이지 너비 동기화
 */
export function normalizeProject(project: ComicProjectData): ComicProjectData {
  if (project.StripWidth > 0) {
    project.StripWidth = clampStripWidth(project.StripWidth);
    for (const page of project.Pages) page.PageWidth = project.StripWidth;
  }
