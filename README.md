...
- 저장/불러오기: `.koma`(프로젝트+이미지 번들) / `.kfjson`(프로젝트만), **IndexedDB 자동저장 + 복원**
...
원본 KomaForge 기능 파리티는 완료 상태다 — 말풍선(구름/폭발·플래시·집중선·속도선·없음, 3점 곡선
꼬리, 외곽선 병합, 텍스트 아웃라인), 칸·이미지(사변형 모서리 모드, 픽셀 알파 히트테스트·클릭 순환),
미디어(움직이는 WebP·동영상, 단축키, 다중 페이지 zip). 세부 대응표는 [UPSTREAM.md](./UPSTREAM.md).
